...
class Particles(RootControl):
    ...
    def do_check(self,noisy_time=False):
        with self.lock:
            ...
            now = time.time()
            if now < self.next_time_particles:
                return

            self.next_time_particles = now + config.SENSOR_PARTICLES_POLL_PERIOD_S
            ...
            if pm1[1] > 100 or pm25[1] > 100 or pm10[1] > 100:
                ...
                if min_diff > 5:  # so it will not alert too frequently
                    ...
                    if not TTS.say("Внимание, высокий уровень частиц в воздухе : "+str(pm25[1]), +" микрограмм на кубометр, надо одеть маску !"):
                        sound.play_command("particles_level")
